...
bool RelExpr(istream& in, int& line, Value & retVal) {
    ...
    if ( tok == LTHAN || tok == GTHAN || tok == EQ)
    {
        ...
        if (!v1.IsInt() && !v1.IsReal()){
            ParseError(line, "Cant compare other than numbers lol");
            return false;
        }
        if (op == LTHAN){
            v = (v<v1);
    ...
    while ( tok == MULT || tok == DIV  || tok == IDIV || tok == MOD)
    {
        ...
        if (op == DIV){
            if (v1.IsReal() && v1.GetReal()==0.0){
                ParseError(line, "Dividing by zero dude ;(");
                return false;
            }
...
bool SFactor(istream& in, int& line, Value & retVal)
{
    ...
    if (sign ==-1 && (!v.IsReal() && !v.IsInt())){
        ParseError(line, "Illegal use of minus :(");
        return false;
    }
    if (sign ==-1 && v.IsReal()){
...
extern Value Value::operator&&(const Value& oper) const{

    if (IsBool() && oper.IsBool()){
        if (GetBool() && oper.GetBool()){
            return true;
        }
        else{
            return false;
...
extern Value Value::operator/(const Value& op) const{
